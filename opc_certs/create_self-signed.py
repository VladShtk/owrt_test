...
os.system("""openssl req \
     ...
     -subj "/C=DE/O=ts_owrt_module/CN=ts_owrt_module@localhost"\
     -out localhost.crt""".format(openssl_conf, keysize))

os.system("openssl x509 -in localhost.crt -outform der -out opc_client_cert.der")
os.system("openssl rsa -inform PEM -in localhost.key -outform DER -out opc_client_key.der")
